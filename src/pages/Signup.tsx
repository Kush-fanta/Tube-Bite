  ...
  return (
    <div className={`flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12 transition-colors duration-300 ${dark ? 'bg-gray-950' : 'bg-gray-50'}`}>
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute top-1/3 left-1/2 h-[500px] w-[500px] -translate-x-1/2 rounded-full bg-fuchsia-500/10 blur-[120px]" />
      </div>
      ...
      <div className={`relative w-full max-w-md rounded-2xl border p-8 shadow-2xl ${dark ? 'border-white/10 bg-gray-900/80 backdrop-blur-xl' : 'border-gray-200 bg-white'
        ...
        <div className="mb-8 text-center">
          ...
          <p className={`mt-1.5 text-sm ${dark ? 'text-gray-400' : 'text-gray-500'}`}>Start creating viral clips today</p>
        </div>

        <button
          onClick={handleGoogle}
          disabled={loading}
          className={`mb-6 flex w-full items-center justify-center gap-3 rounded-xl border py-3 text-sm font-medium transition-colors ${dark ? 'border-white/10 text-white hover:bg-white/5' : 'border-gray-300 text-gray-700 hover:bg-gray-50'
            }`}
        >
        ...
        <div className="relative mb-6">
          <div className="absolute inset-0 flex items-center"><div className={`w-full border-t ${dark ? 'border-white/10' : 'border-gray-200'}`} /></div>
          <div className="relative flex justify-center text-xs uppercase"><span className={`px-3 ${dark ? 'bg-gray-900 text-gray-500' : 'bg-white text-gray-400'}`}>or</span></div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className={`mb-1.5 block text-sm font-medium ${dark ? 'text-gray-300' : 'text-gray-700'}`}>Full Name</label>
            <div className="relative">
              <User className={`absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 ${dark ? 'text-gray-500' : 'text-gray-400'}`} />
              <input
                type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Doe"
                className={`w-full rounded-xl border py-3 pl-10 pr-4 text-sm outline-none transition-colors placeholder:text-gray-500 ${dark ? 'border-white/10 bg-white/5 text-white focus:border-violet-500' : 'border-gray-300 bg-gray-50 text-gray-900 focus:border-violet-500'
                  }`}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full rounded-xl border py-3 pl-10 pr-12 text-sm outline-none transition-colors placeholder:text-gray-500 ${dark ? 'border-white/10 bg-white/5 text-white focus:border-violet-500' : 'border-gray-300 bg-gray-50 text-gray-900 focus:border-violet-500'
                  }`}
              />
